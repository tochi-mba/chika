...
# ── Plan tools tolerate spurious kwargs + aliases ─────────────────────────
...
def test_plan_set_accepts_spurious_kwargs():
    ...
    # The exact crash from the user's session: name= kwarg LLM threw in.
...
def test_plan_set_accepts_alias_for_tasks_kwarg(alias):
    """LLMs commonly call plan_set(steps=[...]) instead of plan_set(tasks=[...]).
    The previous code absorbed `steps` into _extra and crashed with
    'missing required positional argument: tasks'. Now aliases resolve."""
    from chika.skills.plan_skill import _make_plan_tools
    vs = VariableStore()
    plan_set, *_ = _make_plan_tools(vs)
...
def test_plan_set_with_no_tasks_returns_clear_error():
    ...
    # No `tasks`, no aliases → clear error message naming the aliases.
...
def test_plan_update_accepts_aliases(alias_id, alias_status):
    """plan_update(id="t1", state="done") should work just as well as
    plan_update(task_id="t1", status="done")."""
    from chika.skills.plan_skill import _make_plan_tools
    vs = VariableStore()
    ...
    assert "error" not in out, (
        f"plan_update with aliases {alias_id}/{alias_status} should work, got {out}"
    )
    # First task is now done.
...
def test_plan_add_accepts_alias_for_tasks(alias):
    ...
    assert "error" not in out, f"plan_add alias {alias!r} should resolve, got {out}"
...
def test_plan_remove_accepts_alias(alias):
    ...
    assert "error" not in out, f"plan_remove alias {alias!r} should resolve, got {out}"
    assert out["removed"] == ["t1"]


# ── scaffold_web_app tolerates alias kwargs ──────────────────────────────

def test_scaffold_web_app_accepts_alias_kwargs(tmp_path):
    from chika.skills.web_app_skill.scaffold_tool import _scaffold_web_app
    # 'template' as alias for 'stack', 'project_name' as alias for 'name'.
